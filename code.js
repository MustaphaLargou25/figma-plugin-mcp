...
const MAX_PROGRESS_ARRAY_ITEMS = 5;
const MAX_BATCH_EXPORT_CONCURRENCY = 4;
const MAX_EXPORT_PIXELS = 16000000;
const MAX_EXPORT_SCALE = 4;
const COMMAND_SLOW_THRESHOLD_MS = 10000;
...
let mutatingCommandQueue = Promise.resolve();
const commandMetrics = {
  total: 0,
  success: 0,
  failed: 0,
  inFlight: 0,
  queuedMutatingCommands: 0,
  averageDurationMs: 0,
  lastCommand: null,
  lastError: null,
};
...
function compactProgressPayload(payload) {
  if (!payload || typeof payload !== "object") return payload;

  const compact = { ...payload };
  for (const key of ["textNodes", "results", "chunkResults"]) {
    if (Array.isArray(compact[key])) {
      const originalLength = compact[key].length;
      compact[`${key}Count`] = originalLength;
      compact[key] = compact[key].slice(0, MAX_PROGRESS_ARRAY_ITEMS);
      if (originalLength > MAX_PROGRESS_ARRAY_ITEMS) {
        compact[`${key}Truncated`] = originalLength - MAX_PROGRESS_ARRAY_ITEMS;
      }
    }
  }
...
function recordCommandMetric(command, durationMs, ok, errorMessage = null) {
  const completed = commandMetrics.success + commandMetrics.failed + 1;
  commandMetrics.averageDurationMs = Math.round(
    ((commandMetrics.averageDurationMs * (completed - 1)) + durationMs) / completed
  );
  commandMetrics.lastCommand = {
    command,
    durationMs,
    ok,
    timestamp: Date.now(),
  ...
  if (ok) {
    ...
    commandMetrics.failed++;
    commandMetrics.lastError = {
      command,
  ...
  if (durationMs > COMMAND_SLOW_THRESHOLD_MS) {
    console.warn(`[command] ${command} completed in ${durationMs}ms`);
  }
}
...
async function executeCommandMessage(msg) {
  ...
  commandMetrics.inFlight++;

  try {
    const result = await handleCommand(command, msg.params || {});
    const durationMs = Date.now() - startedAt;
    recordCommandMetric(command, durationMs, true);
    figma.ui.postMessage({
      type: "command-result",
      id,
      result,
    ...
    recordCommandMetric(command, durationMs, false, errorMessage);
    figma.ui.postMessage({
      type: "command-error",
      id,
      error: errorMessage,
...
function enqueueMutatingCommand(msg) {
  commandMetrics.queuedMutatingCommands++;
  mutatingCommandQueue = mutatingCommandQueue
    .then(async () => {
      commandMetrics.queuedMutatingCommands--;
      await executeCommandMessage(msg);
    })
    .catch(async (error) => {
      commandMetrics.queuedMutatingCommands = Math.max(0, commandMetrics.queuedMutatingCommands - 1);
      console.error("[command] Queue recovery after error:", error);
      await executeCommandMessage(msg);
    });

  return mutatingCommandQueue;
}
...
function sendProgressUpdate(commandId, commandType, status, progress, totalItems, processedItems, message, payload = null) {
  ...
  if (payload) {
    ...
    update.payload = compactProgressPayload(payload);
  }
...
function getPluginHealth() {
  return {
    ...
    queue: {
      mutatingCommandsQueued: commandMetrics.queuedMutatingCommands,
      inFlight: commandMetrics.inFlight,
    },
    limits: {
      maxBatchExportConcurrency: MAX_BATCH_EXPORT_CONCURRENCY,
      maxExportPixels: MAX_EXPORT_PIXELS,
...
async function exportNodeAsImage(params) {
  let { nodeId, scale = 1, format = "PNG" } = params || {};
  ...
  if (scale > MAX_EXPORT_SCALE) {
    throw new Error(`Scale ${scale} exceeds max supported scale ${MAX_EXPORT_SCALE}`);
  }
  ...
  if ((format === "PNG" || format === "JPG") && "width" in node && "height" in node) {
    const estimatedPixels = Math.ceil(node.width * node.height * scale * scale);
    if (estimatedPixels > MAX_EXPORT_PIXELS) {
      throw new Error(
        `Export would rasterize about ${estimatedPixels.toLocaleString()} pixels. ` +
        `Reduce scale or export a smaller node (limit ${MAX_EXPORT_PIXELS.toLocaleString()} pixels).`
      );
    }
  }
...
function customBase64Encode(bytes) {
  ...
  for (let i = 0; i < mainLength; i = i + 3) {
    ...
    segment += chars[a] + chars[b] + chars[c] + chars[d];
    segmentGroups++;

    if (segmentGroups >= GROUPS_PER_SEGMENT) {
      parts.push(segment);
      segment = "";
      segmentGroups = 0;
    }

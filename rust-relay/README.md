...
The relay listens on `127.0.0.1:3055` by default and keeps the existing WebSocket protocol unchanged, so the current Figma plugin and MCP server can connect without edits.
